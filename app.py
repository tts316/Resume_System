...
class ResumeDB:
    ...
    def get_df(self, table_name):
        # 定義完整欄位 (共 89 欄)，確保與 Google Sheet 一致
        defaults = {
            ...
            "resumes": [
                ...
                "edu_1_start", "edu_1_end", "edu_2_start", "edu_2_end", "edu_3_start", "edu_3_end"
            ],
    ...
    def create_user(self, creator_email, email, name, role, r_type=""):
        try:
            ...
            if role == "candidate":
                # 建立空資料列，欄位數對應 defaults['resumes'] 的長度 (約95欄)
                defaults_len = 95 
                row_data = [email, "New", name] + [""] * (defaults_len - 3)
                # 填入 resume_type (位於第 52 欄, index 51)
                row_data[51] = r_type
                    ...
                    with st.expander(f"{status_badge} {r_badge} {row['name_cn']} ({row['email']})"):
                        ...
                        for x in range(1, 4):
                            ...
                            if s: 
                                ...
                                st.write(f"   - {row.get(f'edu_{x}_major')} | {row.get(f'edu_{x}_degree')} | {row.get(f'edu_{x}_state')}")
                        ...
                        if row.get('resume_type') == 'Branch':
                            ...
                            st.write(f"排班細節: 假日({row.get('holiday_shift')}) | 早晚({row.get('rotate_shift')}) | 家人({row.get('family_support_shift')})")
