...
def candidate_page():
    ...
    with st.form("resume_form"):
        ...
        # [關鍵修正] 使用 container 與 columns 模擬表格
        # 基本資料
        with st.container(border=True):
            st.caption("基本資料")
            c1, c2, c3, c4 = st.columns(4)
        ...
        with st.container(border=True):
            ...
            for i in range(1, 5):
                with st.expander(f"經歷 {i}"):
                    ...
                    ec5.text_input(f"聯絡電話", value=my_resume.get(f'exp_{i}_phone',''), key=f'exp_{i}_phone_in')
        ...
        if r_type == "Branch":
            with st.container(border=True):
                ...
                if shift_val == "是":
                    ...
                    selected_backups = st.multiselect("請勾選可配合輪班的分校 (複選)", backup_opts, default=valid_defaults, key="back_sel")
                    if selected_backups:
                ...
                family_support_shift = st.radio("家人同意輪班？", ["同意", "不同意"], index=get_yn_idx(my_resume.get('family_support_shift')), horizontal=True, key='family_support_shift')
        ...
        with st.container(border=True):
            ...
            # 這裡重複了扶養調查，若 PDF 只有一處可合併，這裡保留為一般調查
            st.radio("獨力扶養？", ["需要", "不需要"], index=get_yn_idx(my_resume.get('family_support')), horizontal=True, key='family_support')
            st.radio("獨力負擔？", ["需要", "不需要"], index=get_yn_idx(my_resume.get('family_debt')), horizontal=True, key='family_debt')
        ...
        # 自動收集 Session State 內的所有 input key
        # 排除系統保留字
        ...
        if c_d.form_submit_button("🚀 送出審核"):
            ...
            elif r_type == "Branch" and shift_val=="是" and "輪班" not in loc_val: st.error("請勾選可配合輪班的分校")
            else:
